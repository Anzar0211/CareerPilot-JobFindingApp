          ...
          <SheetContent side="left" className="bg-white">
            ...
            <div className="grid gap-2 py-6">
              {menuItems.map((item,index)=>(
                item.show ? <Link key={index} className="flex w-full items-center py-2 text-md font-semibold" onClick={()=>sessionStorage.removeItem("filterParams")} href={item.path}>{item?.label}</Link>
                : null
              ))}
            </div>
